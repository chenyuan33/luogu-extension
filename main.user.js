...
(function() {
	'use strict';
	let load;
	if (location.href.startsWith('https://www.luogu.me/')) {
		// TODO
	} else {
		let at;
		if (location.href.startsWith('https://www.luogu.com/')) {
			if (document.title === 'Luogu cannot serve content') {
				return;
			}
			at = location.href.replace('https://www.luogu.com/', '');
		} else {
			at = location.href.replace('https://www.luogu.com.cn/', '');
		}
		if (at === '' || at.startsWith('contest/edit') || at.startsWith('record') || at.startsWith('theme') || at.startsWith('image') || at.startsWith('paste'))
		{
			return;
		}
		load = () => {
			document.getElementsByClassName('user-nav')[0].insertAdjacentHTML('afterbegin', `
				<div id="cy3-ext">
					<img src="//chenyuan33.github.io/favicon.png" alt="cy3" id="cy3-ext-img" />
					<div id="cy3-inner">
						<p><label for="cy3-find-user">搜索用户</label> <input type="text" autocomplete="off" id="cy3-find-user" placeholder="用户名或 UID" /><span id="cy3-find-user-result"></span></p>
					</div>
				</div>
			`);
